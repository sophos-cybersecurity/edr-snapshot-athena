...
SELECT "$path" AS data_source,
         ...
         pidStart,
         tidStart,
         try(cast(from_iso8601_timestamp(time) AS timestamp)) AS time,
         name
...
SELECT "$path" AS data_source,
         ...
         pidStart,
         tidStart,
         try(cast(from_iso8601_timestamp(accessEndTime) AS timestamp)) AS accessEndTime,
         operationType,
         filePath,
         driveType,
         targetFilePath,
...
SELECT "$path" AS data_source,
         type,
         pidStart,
         try(cast(from_iso8601_timestamp(loadTime) AS timestamp)) AS loadTime,
         loadAddress,
         imageSize,
         imagePath,
         driveType,
...
SELECT "$path" AS data_source,
         ...
         pidStart,
         tidStart,
         try(cast(from_iso8601_timestamp(startTime) AS timestamp)) AS startTime,
         try(cast(from_iso8601_timestamp(endTime) AS timestamp)) AS endTime,
         connectionType,
         srcAddress,
         srcPort,
         destAddress,
...
SELECT "$path" AS data_source,
         ...
         pidStart,
         try(cast(from_iso8601_timestamp(startTime) AS timestamp)) AS startTime,
         try(cast(from_iso8601_timestamp(stopTime) AS timestamp)) AS stopTime,
         ppidStart,
         ptidStart,
         processName,
         driveType,
